...
Le microservice est développé en Python \citeURL{python_psf}, choix justifié par l'écosystème mature du traitement du langage naturel et de l'intelligence artificielle. PyTorch \citeURL{pytorch_org}, Transformers \citeURL{huggingface_transformers} et les outils d'adaptation fine-tuning sont des références incontournables dans ce domaine.

Pour l'interface REST, FastAPI \citeURL{fastapi_docs} a été retenu. Ce framework valide automatiquement les données entrantes, génère une documentation interactive et sérialise les réponses en JSON avec un minimum de code. \texttt{Pydantic} \citeURL{pydantic_docs} assure la validation stricte des requêtes et des réponses.
...
\hline
FastAPI & Exposition de l'interface REST \citeURL{fastapi_docs} \\
Pydantic & Validation des requêtes et réponses JSON \\
Transformers & Chargement des modèles pré-entraînés \citeURL{huggingface_transformers} \\
PEFT & Fine-tuning efficace avec LoRA \citeURL{hf_peft} \\
Ollama & Exécution locale du modèle Qwen \citeURL{ollama_com} \\
Langdetect & Détection automatique de la langue \citeURL{langdetect_pypi} \\
Sentence Transformers & Analyse de similarité sémantique \citeURL{sentence_transformers} \\
\hline
\end{tabular}
\end{center}
...
Plusieurs modèles de langage existaient au moment du projet : GPT-4, Claude, Llama, Mistral et Qwen2.5 \citeURL{qwen_model}. Le tableau ci-dessous les compare selon les critères importants pour une utilisation industrielle.
...
Plusieurs modèles de traduction neuronale existent : MarianMT, mBART-50, M2M100 et NLLB \citeURL{nllb_meta}. Le modèle NLLB (No Language Left Behind) de Meta a été sélectionné pour sa couverture de 200 langues, ses performances élevées et sa compatibilité avec le fine-tuning.
...
NLLB \citeURL{nllb_meta} est un modèle Transformer de type Seq2Seq (séquence à séquence). Il prend une phrase en entrée et génère sa traduction en sortie. Le modèle a été entraîné sur des données de traduction couvrant 200 langues.
...
Le score BLEU (Bilingual Evaluation Understudy) \citeURL{bleu_metric} est une métrique de référence qui mesure la similarité n-grammatique entre la traduction générée par le modèle et une traduction de référence produite par un humain.

L'évaluation sur 7 langues (30 échantillons chacune) donne un score BLEU moyen de 28.4, en progression significative par rapport aux 23,5 du modèle de base. Bien que modeste face aux 35 BLEU des modèles les plus performants, ce score confirme que le fine-tuning LoRA \citeURL{hf_peft} sur un jeu de 5 000 paires adapte efficacement le modèle au vocabulaire technique et au style des fiches produits, validant ainsi son déploiement en production.
...
\item \textbf{Détection automatique de la langue :} la bibliothèque \texttt{langdetect} \citeURL{langdetect_pypi} identifie la langue réellement utilisée dans chaque contenu textuel afin de vérifier qu'elle correspond à la langue attendue pour le champ concerné. Une incohérence peut révéler une erreur de traduction, une mauvaise affectation des contenus multilingues ou une inversion des langues.

\item \textbf{Analyse grammaticale et orthographique :} les textes sont analysés à l'aide de la bibliothèque \texttt{LanguageTool} \citeURL{languagetool_org}, qui détecte les erreurs d'orthographe, de grammaire, de ponctuation et certaines maladresses de formulation susceptibles de diminuer la qualité éditoriale de la fiche produit.

\item \textbf{Analyse contextuelle :} certaines anomalies ne peuvent être détectées uniquement à partir de règles grammaticales. Le modèle de langage Qwen2.5 \citeURL{qwen_model} est donc sollicité afin d'évaluer le contenu dans son contexte et d'identifier des formulations peu naturelles, des incohérences rédactionnelles ou des expressions pouvant être améliorées tout en conservant le sens initial du texte.
...
\item \textbf{Corrections automatiques :} elles concernent les anomalies de faible risque pouvant être résolues sans intervention humaine. Ces corrections améliorent la qualité technique des données sans modifier leur signification. Elles comprennent notamment le nettoyage des balises HTML incorrectes à l'aide de \texttt{BeautifulSoup} \citeURL{bs4_pypi}, la suppression des espaces superflus, la suppression des URLs locales ou invalides, la conversion des liens \texttt{http} vers \texttt{https} ainsi que la normalisation du format des unités techniques et de certains caractères. Une fois appliquées, ces modifications sont enregistrées directement dans SAP Commerce Cloud afin d'améliorer la qualité des fiches produits.
