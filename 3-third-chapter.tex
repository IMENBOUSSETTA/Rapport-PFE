...
Plusieurs modèles de langage existaient au moment du projet : GPT-4, Claude, Llama, Mistral et Qwen2.5~\citeURL{llm_models, qwen2_5}. Le tableau~\ref{tab:comparaison_llm} les compare selon les critères importants pour une utilisation industrielle.
...
À partir de ces informations, le modèle génère automatiquement une description respectant les contraintes définies dans le prompt. Le résultat obtenu pour cet exemple est présenté ci-dessous :

\begin{tcolorbox}[
    colback=gray!5,
    colframe=black,
    title=Résultat généré par le modèle,
    fonttitle=\bfseries
]
Le moteur électrique est une solution de motorisation industrielle conçue pour assurer l'entraînement de différents équipements et installations. Avec une puissance de 50 W et une tension nominale de 220 V, il convient aux applications nécessitant une motorisation fiable et adaptée aux environnements industriels. 
\end{tcolorbox}
...
Plusieurs modèles de traduction neuronale existent : MarianMT, mBART-50, M2M100 et NLLB~\citeURL{translation_models, nllb}. Le modèle NLLB (\textit{No Language Left Behind}) de Meta a été sélectionné pour sa couverture de 200 langues, ses performances élevées et sa compatibilité avec le fine-tuning.
...
NLLB~\citeURL{nllb} est un modèle Transformer de type Seq2Seq (séquence à séquence). Il prend une phrase en entrée et génère sa traduction en sortie. Le modèle a été entraîné sur des données de traduction couvrant 200 langues.
...
Le score BLEU (\textit{Bilingual Evaluation Understudy})~\citeURL{bleu_score} est une métrique de référence qui mesure la similarité n-grammatique entre la traduction générée par le modèle et une traduction de référence produite par un humain.

L'évaluation sur 7 langues (30 échantillons chacune) donne un score BLEU moyen de 28,4, en progression significative par rapport aux 23,5 du modèle de base. Bien que modeste face aux 35 BLEU des modèles les plus performants, ce score confirme que le fine-tuning LoRA~\citeURL{lora} sur un jeu de 5\,000 paires adapte efficacement le modèle au vocabulaire technique et au style des fiches des produits, validant ainsi son déploiement en production.
...
À l'issue du processus d'enrichissement, le microservice retourne une réponse structurée contenant les informations générées ainsi que les métadonnées nécessaires à leur intégration dans SAP Commerce Cloud.
...
La Figure~\ref{fig:pipeline_validation} présente l'organisation du flux de validation sous la forme d'un pipeline composé de plusieurs couches successives.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{flux-validation.png}
    \caption{Flux de validation des produits}
...
\begin{itemize}
    \item \textbf{Détection automatique de la langue :} la bibliothèque \texttt{langdetect}~\citeURL{langdetect} de Python identifie la langue réellement utilisée dans chaque contenu textuel afin de vérifier qu'elle correspond à la langue attendue pour le champ concerné. Une incohérence peut révéler une erreur de traduction, une mauvaise affectation des contenus multilingues ou une inversion des langues.

    \item \textbf{Analyse grammaticale et orthographique :} les textes sont analysés à l'aide de la bibliothèque LanguageTool~\citeURL{languagetool} de Python, qui détecte les erreurs d'orthographe, de grammaire, de ponctuation et certaines maladresses de formulation susceptibles de diminuer la qualité éditoriale de la fiche du produit.

    \item \textbf{Analyse contextuelle :} certaines anomalies ne peuvent être détectées uniquement à partir de règles grammaticales. Le modèle de langage Qwen2.5~\citeURL{qwen2_5} est donc sollicité afin d'évaluer le contenu dans son contexte et d'identifier des incohérences rédactionnelles ou des expressions pouvant être améliorées tout en conservant le sens initial du texte.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Corrections automatiques :} elles concernent les anomalies de faible risque pouvant être résolues sans intervention humaine. Ces corrections améliorent la qualité technique des données sans modifier leur signification. Elles comprennent notamment le nettoyage des balises HTML incorrectes à l'aide de BeautifulSoup~\citeURL{beautifulsoup}, la suppression des espaces superflus, la suppression des URLs locales ou invalides, la conversion des liens \texttt{http} vers \texttt{https} ainsi que la normalisation du format des unités techniques et de certains caractères. Une fois appliquées, ces modifications sont enregistrées directement dans SAP Commerce Cloud afin d'améliorer la qualité des fiches des produits.
...
\subsection{Couche 4 : Agrégation des résultats et calcul du score de qualité}
...
Ce chapitre a présenté la conception et le fonctionnement du microservice d'intelligence artificielle développé dans le cadre de ce projet. Après avoir décrit son architecture et les technologies retenues, les principaux modules ont été détaillés, notamment la génération des descriptions, la traduction multilingue et la validation des fiches des produits.

Les mécanismes mis en œuvre combinent des modèles d'intelligence artificielle et des règles de validation afin d'automatiser les opérations d'enrichissement tout en garantissant la cohérence et la fiabilité des données. L'organisation du module de validation en plusieurs couches permet de vérifier progressivement la structure, le contenu et la qualité des données générées.

Le microservice ainsi conçu est modulaire, indépendant de SAP Commerce Cloud et évolutif. Il expose des interfaces REST permettant d'assurer une communication standardisée avec la plateforme.

Le chapitre suivant présente l'intégration de ce microservice dans SAP Commerce Cloud, ainsi que les développements réalisés et les mécanismes d'échange mis en place entre les deux composants.
